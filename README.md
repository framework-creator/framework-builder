# Framework Builder

A systematic methodology for building reusable, agent-readable frameworks from any domain expertise.

Built by [Mike Goetz](https://ragedesigner.com). 1,000+ frameworks across industries since 2024.

## What This Is

A single `SKILL.md` file that teaches AI agents (and humans) how to build frameworks. It works in Claude Code, Claude Projects, Cursor, ChatGPT, Copilot, or any environment that supports skills or custom instructions.

The methodology produces five-layer frameworks that both agents and humans can follow for consistent results across any domain.

## Why It Exists

The AI ecosystem is converging on "skills" (markdown files that give AI context for specific tasks). Individual skills are useful, but they don't compound without systematic methodology.

This skill teaches the meta-methodology: how to generate frameworks across any domain. While everyone else is trading individual skills like baseball cards, this teaches you how to build the card factory.

Built from experience creating 1,000+ frameworks across federal contracting, sales methodology, medical advocacy, video production, content automation, and dozens of other industries.

## How to Use It

1. Drop `SKILL.md` into your Claude Project, Cursor rules, or any AI skill directory
2. Tell your AI: "Build a framework for [your domain/problem]"
3. The AI uses the five-layer DNA architecture to produce a complete, reusable framework

That's it. One file, any domain, consistent results.

## The Five Layers

Every framework generated by this methodology follows the same DNA:

- **Layer 1: Principles Foundation** - The WHY. Evidence-backed core truths, not opinions.
- **Layer 2: Systematic Approach** - The HOW. Reasoning steps with decision logic, not just procedures.
- **Layer 3: Force Multipliers** - The LEVERAGE. What creates outsized results from small inputs.
- **Layer 4: Success Metrics** - The MEASUREMENT. What good looks like, and what broken looks like.
- **Layer 5: Implementation Guidance** - The DEPLOYMENT. Edge cases, context adaptation, common mistakes.

## Examples

See the [`examples/`](examples/) folder for sample frameworks generated by this methodology.

### Industry Examples

Worked examples of frameworks built for specific industries:

- **[Delegation Brief Framework](examples/example-delegation-brief-framework.md)**. The framework built across the five-day "five layers" series, one layer at a time. A complete worked example of the five-layer DNA, including the Implementation layer that turns a framework from a document into working infrastructure. Start here if you came from the series.
- **[Film & Audio](examples/film-audio/)**. Audio Visual Production Intelligence and related frameworks for sound engineers, film professionals, and post-production teams.

More examples coming as the open-source library grows. To learn about the methodology behind these frameworks, see [whatisaframework.com](https://whatisaframework.com).

Also see [TRIGGER-MAP.md](TRIGGER-MAP.md) for when to reach for each framework in the companion meta-cognition series built by Grok (xAI).

## What the Framework Builder Includes, and What It Doesn't

The framework builder skill file teaches any AI how to generate a properly structured five-layer framework. It handles the architecture. The shape of what a framework should look like, what belongs in each layer, and how to make it agent-readable.

What it does not include is the intelligence library that makes SIOS-quality frameworks different from well-structured generic ones.

### What's missing from the open source release

**The Force Multiplier Catalog.** A force multiplier is the mechanism that makes a framework compound beyond its basic application. Generic force multipliers like "automation," "delegation," and "leverage" are easy to generate. The SIOS catalog contains specific, cross-domain sourced force multipliers with exact mechanisms, documented failure modes, and known interaction effects with other frameworks. When an AI generates a framework without this catalog, it defaults to general examples. When it uses the catalog, the force multipliers are precise, non-obvious, and systematically chosen.

### What this means in practice

If you use the framework builder skill file to generate a five-layer framework, you will get a well-structured, agent-readable document with coherent principles, a systematic approach, and reasonable force multipliers. That is genuinely useful and significantly better than a prompt.

If you generate the same framework using the full SIOS methodology, the force multipliers will be specific mechanisms drawn from adjacent domains, chosen systematically rather than generated from general knowledge.

The open source release is the architecture. The methodology is what fills it.

### A note on AI-generated frameworks

Any AI using this skill file will produce frameworks based on its training data and whatever context you provide. The quality ceiling without the SIOS catalogs is the general knowledge of the model. The quality ceiling with the catalogs is the accumulated cross-domain intelligence of 1,000+ systematically built frameworks. Both are real. They are just different instruments.

## What This Doesn't Cover

This is the foundational framework generation process. For the full ecosystem:

- **Advanced multi-framework orchestration:** [ragedesigner.com](https://ragedesigner.com), Strategic consulting and the complete SIOS framework library (1,000+ frameworks)
- **Learn the methodology:** [howtoframework.com](https://howtoframework.com), Practical guides for building frameworks from your own domain expertise
- **Understand what frameworks are:** [whatisaframework.com](https://whatisaframework.com), The foundational concepts behind systematic thinking
- **Find frameworks by use case:** [whereframeworks.com](https://whereframeworks.com), Browse frameworks organized by industry and problem type
- **Courses and learning paths:** [whenframework.com](https://whenframework.com), Structured courses including Love Your Limits (constraint-driven innovation) and Reverse Engineering Success

## License

[MIT License](LICENSE). Use it, share it, build on it.

## About

Mike Goetz is the founder of [RageDesigner](https://ragedesigner.com) (est. 2003) and creator of the Strategic Intelligence Operating System (SIOS). He has built 1,000+ frameworks across unlimited industries using this methodology since 2024. His work bridges the gap between academic research and practical business application through systematic framework generation.
